...
const Decoration = ({ idx, nickname, imageIdx }: DecorationType) => {
    ...
    const onClick = async () => {
        try {
            const fetchDeco = await fetchDecoration(treeIdx, idx, user.jwt);
    ...
    return (
        <VStack>
            <IconButton aria-label="Decoration" bgColor="transparent" _hover={{ bgColor: "transparent" }} onClick={onClick} icon={
